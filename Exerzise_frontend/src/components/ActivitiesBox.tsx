  ...
  return (
    <div className="flex flex-col mx-auto md:flex-row justify-center m-20 h-[800px] md:h-[400px] w-[350px] md:w-[1000px]">
      <div className="w-full bg-slate-200 text-center p-4 relative z-3">
        <div className="tabs-container absolute flex flex-col left-[-30px] top-0 h-[400px] justify-around">
          {tabs.map((el, index) => (
            <div
              onClick={() => {
                setAct(el);
              ...
              key={index}
              className={`cursor-pointer flex items-center tab h-[50px] w-[100px] hover:-translate-x-12 ${
                el.name === act.name ? "bg-green-100" : "bg-slate-100"
              }`}
            >
              <img src={el.icon} alt="" className="h-[20px] mx-4" />
            </div>
          ))}
        </div>
      ...
      <div className="w-full h-full bg-red-200 text-center p-4 relative">
        <header className="p-10">{act.content}</header>
      </div>
    </div>
  );
};
